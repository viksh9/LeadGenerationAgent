"""Opportunity analysis for LeadGenerationAgent.

Two APIs live here:

* ``OpportunityAnalyzer`` (current) — a deterministic engine that turns a
  ``SignalDetectionResult`` + lead data into an IT-services/staffing opportunity
  assessment (type, staffing need, roles, team size, urgency, confidence).
* ``analyze_opportunity`` / ``OpportunityInsight`` — the earlier buying-stage
  narrative helper, kept intact because other intelligence modules import it.
"""
