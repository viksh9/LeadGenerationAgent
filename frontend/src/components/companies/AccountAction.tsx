...
export function AccountAction({ company }: { company: CompanyIntelligence }) {
  ...
  return (
    <section className="card card-pad" aria-label="Recommended account action">
      ...
      <p className="mt-2 text-sm leading-relaxed text-slate-700">{company.recommendedAction}</p>
      ...
      <dl className="mt-3 space-y-2 border-t border-slate-100 pt-3">
        <div className="flex items-center justify-between">
          <dt className="text-xs uppercase tracking-wide text-slate-400">Account priority</dt>
          <dd>
            <PriorityBadge priority={company.topPriority} />
          </dd>
        </div>
        <div className="flex items-center justify-between gap-3">
          <dt className="text-xs uppercase tracking-wide text-slate-400">Suggested POC</dt>
          <dd className="truncate text-right text-sm text-slate-700">{topRole ?? 'Not available'}</dd>
        </div>
      </dl>
    </section>
  );
