...
const STATUS_CLASS: Record<OutreachStatus, string> = {
  DRAFT: 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 ring-slate-200 dark:ring-slate-700',
  ...
  COMPLETED: 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 ring-slate-200 dark:ring-slate-700',
};
...
export function MessageStrategyBadge({ strategy }: { strategy: MessageStrategy }) {
  return <span className="badge bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">{MESSAGE_STRATEGY_LABELS[strategy]}</span>;
}
...
export function PitchConfidence({ confidence }: { confidence: number | null }) {
  if (confidence == null) return <span className="text-sm text-slate-400 dark:text-slate-500">Not available</span>;
  const band = confidence >= 75 ? 'HIGH' : confidence >= 50 ? 'MEDIUM' : 'LOW';
  return (
    <span className="inline-flex items-center gap-2">
      <span className="text-sm font-semibold tabular-nums text-slate-700 dark:text-slate-300">{Math.round(confidence)}%</span>
      <span className="text-xs font-medium text-slate-500 dark:text-slate-400">{band}</span>
    </span>
  );
}
