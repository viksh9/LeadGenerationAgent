  ...
  return (
    <section className="card card-pad" aria-label="Recommended action">
      ...
      {lead.recommended_action ? (
        <>
          <p className="mt-2 text-sm leading-relaxed text-slate-700 dark:text-slate-300">{lead.recommended_action}</p>
          <div className="mt-2 flex justify-end">
            <CopyButton text={lead.recommended_action} label="Copy recommended action" />
          </div>
        </>
      ) : (
        <p className="mt-2 text-sm text-slate-400 dark:text-slate-500">Not available</p>
      )}

      <dl className="mt-3 space-y-2 border-t border-slate-100 dark:border-slate-800 pt-3">
        <div className="flex items-center justify-between">
          <dt className="text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500">Priority</dt>
          <dd>
            <PriorityBadge priority={lead.lead_priority} />
          </dd>
        </div>
        <div className="flex items-center justify-between gap-3">
          <dt className="text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500">Recommended POC</dt>
          <dd className="truncate text-right text-sm text-slate-700 dark:text-slate-300">{poc || 'Not available'}</dd>
        </div>
      </dl>
    </section>
  );
