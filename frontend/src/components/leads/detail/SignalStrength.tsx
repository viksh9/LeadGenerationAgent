...
function band(value: number): Band {
  ...
  return { label: 'WEAK', className: 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 ring-slate-200 dark:ring-slate-700' };
}

export function SignalStrength({ confidence }: { confidence: number | null | undefined }) {
  if (confidence === null || confidence === undefined) {
    return <span className="text-sm text-slate-400 dark:text-slate-500">Not available</span>;
  }
  ...
  return (
    <span className="inline-flex items-center gap-2">
      ...
      <span className="text-sm font-semibold tabular-nums text-slate-700 dark:text-slate-300">
        {value}
        <span className="font-normal text-slate-400 dark:text-slate-500"> / 100</span>
      </span>
    </span>
  );
}
