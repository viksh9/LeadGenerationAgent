"""Deterministic, template-based multi-channel pitch generation.

``PitchGenerator`` drafts email / LinkedIn / call-talking-point outreach from the
analysis outputs. No LLM, no network, no database, no sending.
"""
...
def _extract_target_role(poc: Any, lead_data: Any) -> Optional[str]:
    # Works for POCFinder (primary_role.role) or a contact-style poc (primary_contact.title).
